...
void UpdateWeaponInfo::operator()(int update_period_ms)
{
    while (true)
    {
        ...
        memory::ReadMem(module::csgo_process_handle, game::player_entity_address_list[game::local_player_index].address_ + offsets::m_hActiveWeapon, game::curr_weapon_entity_index);

        //kepe the first 3 bytes
        game::curr_weapon_entity_index &= 0xfff;

		//note: this is entity, so we may be anble to render it
        memory::ReadMem(module::csgo_process_handle, module::client_dll + offsets::dwEntityList + (game::curr_weapon_entity_index - 1) * sizeof(EntityAddress), game::curr_weapon_entity_address);

        //normalize the weapon definition index range
        short temp_curr_weapon_definition_index;
        memory::ReadMem(module::csgo_process_handle, game::curr_weapon_entity_address + offsets::m_iItemDefinitionIndex, temp_curr_weapon_definition_index);
