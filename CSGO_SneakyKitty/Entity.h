...
private:
	...
	uint8_t pad0_;
	bool dormant_;//237
	uint8_t pad1_[0xF4 - 0xED - sizeof(dormant_)]; //m_iTeamNum - m_bDormant - sizeof(dormant_)
	int team_; //244
	uint8_t pad2_[0x100 - 0xF4 - sizeof(team_)]; //m_iHealth - m_iTeamNum - sizeof(team_)
	int health_; //256
	int flag_;
	Position view_offset_;
	Vector3 velocity_; //275
	uint8_t pad3_[0x138 - 0x114 - sizeof(velocity_)]; //m_vecOrigin - m_vecVelocity - sizeof(velocity_)
	Position origin_; //312
	uint8_t pad4_[0x93D - 0x138 - sizeof(origin_)]; //m_bSpotted - m_vecOrigin - sizeof(origin_)
	bool spotted_;
	uint8_t pad5_[0x980 - 0x93D - sizeof(spotted_)]; //m_bSpottedByMask - m_bSpotted - sizeof(spotted_)
